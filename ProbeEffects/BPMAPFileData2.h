#ifndef __BPMAPFileData2_h_included__
#define __BPMAPFileData2_h_included__
...
{
	unsigned int	PMX;	/*! The X coordinate of the PM probe. */
	unsigned int	PMY;/*! The Y coordinate of the PM probe. */
	unsigned int	MMX;	/*! The X coordinate of the MM probe. */
	unsigned int	MMY;	/*! The Y coordinate of the MM probe. */
	float			MatchScore;/*! The match score for the PM probe. */
	unsigned int	Position;/*! The genomic position the probe is interrogating. */	
	std::string		PMProbe;	/*! The PM probe sequence. */
	unsigned char	ProbeLength;	/*! The length of the PM probe. */
	...
	bool operator<(const _GDACSequenceHitItemType &rhs) const;/*! Comparison operator used by sort */
	char PackedPMProbe[PROBE_STORAGE_BUFFER_LENGTH+1];	/*! Packed PM probe sequence */
...
{
public:
	CGDACSequenceItem();	/*! Constructor */
	~CGDACSequenceItem();	/*! Destructor */
protected:  
	std::string m_Name;	/*! The name of the sequence, e.g., chromosome. */
	std::string m_GroupName;	/*! The group name for the sequence, e.g., species or organism. */
	std::string m_SeqVersion;	/*! The version associated to the sequence, e.g., build version of the genomic database. */
	int m_Number;		/*! The sequence id. */
	int m_ProbePairs;	/*! Flag indicating if PM/MM pairs. */
	int m_NumberHits;	/* The number of probes or probe pairs (hits). */
	std::vector<GDACSequenceHitItemType> m_Hits;		/*! An array of hits. */
	std::vector<GDACSequenceHitItemType> *m_pHits;	/*! A pointer to the array of hits. This is used in memory mapping files. */
	TagValuePairTypeVector m_Parameters;	/*! The parameter name/value array. */
	bool m_bMapped;	/*! Flag indicating if the file is memory mapped. */
	char *m_lpData;	/*! Pointer to the memory mapped file. */
	int m_HitStartPosition;	/*! The file position of the start of the data. */
...
public:
	...
	friend class CBPMAPFileData2;
};


class CBPMAPFileData2
{
public:
	CBPMAPFileData2(void);
	~CBPMAPFileData2(void);
protected:
	std::string m_FileName;	/*! The full path to the BPMAP file. */
	int	 m_NumberSequences;	/*! The number of sequences stored in the BPMAP file. */
	float m_Version;	/*! The version number of the BPMAP file. */
	int m_DataStartPosition;		/*! The file position of the start of the data. */
	std::string m_strError;	/*! A string to hold error messages. */
	std::vector<CGDACSequenceItem> m_SequenceItems;	/*! An array of sequence items - used when not  memory mapping. */
	...
	void  *m_lpFileMap;	/*! Pointer to a memory mapped file. */
	char  *m_lpData;	/*! Pointer to the data in the memory mapped file. */
#ifdef WIN32
	HANDLE m_hFileMap;	/*! Windows handle to the file. */
	HANDLE m_hFile;/*! Windows handle to the file. */
#else
	FILE *m_fp;/*! File pointer for memory mapping. */
	int m_MapLen;/*! The size of the file. */
#endif
	bool m_bFileOpen;	/*! Flag indicating if the file is open. */
	bool m_bFileMapped;	/*! Flag indicating if the file is memory mapped. */
