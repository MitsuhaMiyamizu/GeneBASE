...
std::string StringTokenizer::nextToken()
{
   ...
   std::string  tmp_str = "";
	string::size_type pos     = token_str.find(delim,0);
...
unsigned long int StringTokenizer::nextUnsignedLongIntToken() 
{
	char* pEnd;
	return strtoul( nextToken().c_str(), &pEnd, 10);
}
...
std::string StringTokenizer::nextToken(const std::string& delimiter)
{
   ...
   std::string  tmp_str = "";
	string::size_type pos     = token_str.find(delimiter,0);
